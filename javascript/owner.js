...
const processQueue = async () => {
    if (isProcessingQueue || !navigator.onLine) {
        if (!navigator.onLine) console.log('Queue processing paused: App is offline.');
        return;
    }
    ...
    try {
        ...
        console.error('API Queue Error:', e.message, '. Pausing queue.');
        isProcessingQueue = false;
...
async function saveCalcToSheet(janjiSelesai = null){
    ...
    el.pcBody.querySelectorAll('tr').forEach(row => { 
        payload.items.push({ 
            ...
            harga_satuan: row.cells[3]?.textContent || '0', 
            jumlah: row.cells[4]?.textContent || '0' 
        }); 
    }); 
    ...
    if (el.orderList && el.orderPlaceholder) { 
        ...
        el.orderList.insertAdjacentHTML('beforeend', `<li>${jam.substring(0,5)} - <b>${payload.nama_tx}</b> (${IDR(payload.harga)}) - ${payload.metode}</li>`); 
    } 
...
function updateWitaTime() { const { tanggal, jam, hariNama } = witaDateTime(); const dateParts = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Makassar', weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).formatToParts(now()); const pick = (parts, t) => parts.find(p => p.type === t)?.value || ''; if (el.pcTanggal) el.pcTanggal.textContent = `${pick(dateParts, 'weekday')}, ${pick(dateParts, 'day')} ${pick(dateParts, 'month')} ${pick(dateParts, 'year')}`; if (el.pcPukul) el.pcPukul.textContent = `${jam} WITA`; if (el.liveWitaTime) el.liveWitaTime.textContent = `${hariNama}, ${pick(dateParts, 'day')} ${pick(dateParts, 'month')} ${jam} WITA`; }
...
if(el.btnStart) el.btnStart.addEventListener('click',async()=>{ 
    ...
    if(!confirm(`Mulai shift dengan Saldo Awal ${fmtDots(s)}?`)) return; 
    const {tanggal:t,jam:j}=witaDateTime(); 
    const u={date:t,start_at:j,end_at:null,start_timestamp:now().toISOString(),end_timestamp:null,cashier:CASHIER_NAME,saldo_awal:s,total_cash:0,n_cash:0,total_qris:0,n_qris:0,total_exp:0,uang_real:0,selisih:0,transactions:[],expenses:[]}; 
